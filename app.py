...
with tab0:
    col1, col2=st.columns(2)
    with col1:
        st.write('##### Projeção de preço - 15 dias')
        forecast2 = m2.predict(future2)
        forecast3=forecast2[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].sort_values(by="ds",ascending=False)           
        forecast3.columns = ['Data', 'Previsão', 'Limite Inferior', 'Limite Superior']
        forecast3['Data'] = pd.to_datetime(forecast3['Data']).dt.date
...
with tab0:
    ...
    with col4:
        st.write('##### Range previsto (15 dias)')
        fig2 = px.line(forecast2.tail(15), x='ds', y='yhat', labels={'yhat': 'Preço do petróleo Brent (US$)'})
        fig2.show()
...
with tab0:
    with col3:
        ...
        st.write('''
                <br>                
    ➡️ Podemos observar no gráfico "Previsto x Real" (acima), que o modelo prevê um aumento no preço do petróleo Brent para os próximos 15 dias.  
                 A linha azul representa a média prevista, enquanto a faixa em azul claro representa o intervalo de confiança.  
                <br>
                <br>  
                                
    ➡️ O gráfico ao lado é um recorte que contempla o período de 15 dias previsto pelo modelo para o preço do petróleo Brent, com base nos dados históricos dos últimos 10 anos.  
...
with tab1:
    ...
    with col3:
        ...
        st.dataframe(performance2, use_container_width=True)
...
with tab1:
    ...
    #### Validação do modelo
     ...
     ➡️ mape: média dos valores absolutos dos erros percentuais.  
                 * No modelo apresentado o erro percentual médio no período pode ser considerado relativamente baixo, entre 10 e 13%.
...
with tab2:
    ...
    * aumento significativo da produção de petróleo nos Estados Unidos.
    * OPEP manteve sua produção em níveis elevados para manter sua participação de mercado e pressionar os produtores de petróleo de xisto dos EUA.  
...
with tab3:
    ...
    🔷 Modelo treinado com dados históricos dos últimos 10 anos, e prevê o preço do petróleo Brent para os próximos 15 dias.  
            
    🔷 O modelo apresenta um erro percentual médio entre 8 e 17%.  
            
    🔷 Ao todo foram testados 3 modelos, todos utilizando o Prophet, cada um com um ajuste de parâmetros diferente.  
               
    1º modelo: testado com a base total, desde o início da medição em 1987.  
    2ª modelo: considera apenas os últimos 10 anos da base histórica.  
    3º modelo: excluído o range correspondente ao período da pandemia.  
    Dentre os modelos, aquele que apresentou melhor desempenho e ajuste foi o segundo.  
    ...
    inicialmente foi testado com 90 dias, depois 15 e por fim 120 dias, sendo que o modelo que apresentou melhor desempenho foi o de 15 dias.  
